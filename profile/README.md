...
Copy the `names.txt` file generated by [Pegasus-Traffic](#1-optimal-parsable-name-format), along with `fw-ndntestbed.py` and `fw-ndntestbed.sh` from Pegasus-Server, to the `ndn-dpdk/sample/activate` directory. Then, offload the FIB entries, activate, and start the forwarder:
```shell
